...
OBJECT::OBJECT ( const char * data , int size ) : size(size)
{
...
bool OBJECT::generate_best_children ()
{
	...
	{
		swap( 	(this + 1)->usporadani[mezera_index] , // posun
			(this + 1)->usporadani[mezera_index-1] );
		// vyhodnot
	...
	{
		swap( 	(this + 1)->usporadani[mezera_index] ,
			(this + 1)->usporadani[mezera_index + size] );
...
static unsigned int heuristic_3x3 ( const unsigned char * data )
{
	...
	for ( int pozice = 0 ; pozice < 3 * 3 ; ++pozice )
	{
		...
		{
			value += -(pozice-1)%3 +2; // horizontalni
			value += -(pozice-1)/3 +2; // vertikalni
			continue;
		}
